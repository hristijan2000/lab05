...
char tocniPrvi[4]={'b', 'c', 'b', 'b'};
char tocniVtori[4]={'b', 'a', 'c', 'a'};
char tocniTreti[4][2]={{'b', 'd'}, {'b', 'd'}, {'b', 'd'}, {'c', 'd'}};
...
int main(){
    ...
    if(file.is_open()){
        cout<<"Ne moze po vtor pat da resavate!"<<endl;
    }else{

    file.open(filename.c_str(),ios::out);
    file.close();
    fstream st ("Students.log", ios::app);

    broj++;
    time (&t);
    timeinfo = localtime (&t);
    string vreme = asctime(timeinfo);
          st << broj << ". " << asctime(timeinfo);
          st << " " << lice.ime << " " << lice.prezime << " INKI" << lice.indeks <<endl;
          st.close();
    // Vnesuvanje na vremeto od pocetokot na testot

    test.open("../prashanja.txt", ios::in);
    while(getline( test , prashanja )){
      for (int i=0; i < prashanja.size();i++){
        if (prashanja[i] == ';'){
          cout<<endl;
          i++;
        }
    ...
    timeinfo = localtime (&t);
    file.open(filename.c_str(), ios::app);
    file<<"Vashata ocenka: "<<score(poeni)<<endl;
    file<<"\n-------------------------\n\nIme: "<<lice.ime<<"\nPrezime: "<<lice.prezime<<"\nIndeks: INKI"<<lice.indeks<<endl;
    file<<"Vreme na start: "<<vreme<<"Vreme na kraj: "<<asctime(timeinfo)<<endl;
    file.close();
    return 0;
